...
const ListingDetail = (props) => {
  ...
  const [resellar, setResellar] = useState({});
  ...
  useEffect(() => {
    ...
    if (id) {
      axios
        .get(`${process.env.REACT_APP_API_URL}/api/resellars/${id}`, config)
        .then((res) => {
          setResellar(res.data);
        })
        .catch((err) => {});
    }
  }, [listing.resellar]);
  ...
  return (
    <div className='listingdetail'>
      <Helmet>
        <title>Farm Produce - Listing | {`${listing.title}`}</title>
      ...
      <div className='row'>
        ...
        <div className='col-1-of-4'>
          ...
          <h3 className='listingdetail__realtor'>{resellar.name}</h3>
          <p className='listingdetail__contact'>{resellar.phone}</p>
          <p className='listingdetail__contact'>{resellar.email}</p>
          <p className='listingdetail__about'>{resellar.description}</p>
        </div>
      </div>
      <div className='row'>
        <div className='col-1-of-2'>
          <ul className='listingdetail__list'>
            <li className='listingdetail__list__item'>
              Produce Type: {listing.produce_type}
            </li>
